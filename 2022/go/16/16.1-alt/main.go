...
func processValve(curr *Valve, time int, workingValves []*Valve, openedValves []*Valve) int {
	var maxValue int
	for _, valve := range workingValves {
		...
		if !alreadyOpened {
			newTime := time - (curr.minPaths[valve.name] + 1)
			if newTime > 0 {
				val := valve.flow*newTime + processValve(valve, newTime, workingValves, append(openedValves, valve))
				if val > maxValue {
					maxValue = val
				}
			}
...
func main() {
	...
	if valveA != nil {
		...
		max := processValve(valveA, time, workingValves, []*Valve{valveA})
		println(max)
	}
}
